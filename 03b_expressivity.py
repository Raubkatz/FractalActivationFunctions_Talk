...
FONT_SCALER = 1.0
LINEWIDTH_SCALER = 1.0


def _fs(x: float) -> float:
    return float(x) * float(FONT_SCALER)


def _lw(x: float) -> float:
    return float(x) * float(LINEWIDTH_SCALER)
...
sns.set_theme(
    ...
    rc={
        "figure.dpi": 150,
        "axes.titlesize": _fs(14),
        "axes.labelsize": _fs(12),
        "lines.linewidth": _lw(2.5),
    },
)
...
def main() -> None:
    ...
    for act_name, act_layer in ACTIVATIONS.items():
        ...
        sns.lineplot(x=dep, y=lengths, marker="o", ax=ax1, linewidth=_lw(2.5))
        ax1.set_title(f"Trajectory length vs. depth – {act_name}", fontsize=_fs(14))
        ax1.set_xlabel("Layer depth d", fontsize=_fs(12))
        ax1.set_ylabel(r"Total arc-length $L_d$", fontsize=_fs(12))
        ax1.tick_params(axis="both", which="both", labelsize=_fs(10), width=_lw(0.8))
        save_plot(fig1, f"length_{act_name}")

        # ---- 2) Region transitions per depth ------------------------------
        if not np.isnan(transitions).all():
            fig2, ax2 = plt.subplots(figsize=(5, 3))
            sns.lineplot(x=dep, y=transitions, marker="o", ax=ax2, linewidth=_lw(2.5))
            ax2.set_title(f"Activation transitions – {act_name}", fontsize=_fs(14))
            ax2.set_xlabel("Layer depth d", fontsize=_fs(12))
            ax2.set_ylabel("# transitions along trajectory", fontsize=_fs(12))
            ax2.tick_params(axis="both", which="both", labelsize=_fs(10), width=_lw(0.8))
        ...
        for ax, idx in zip(axes, SNAPSHOTS):
            ...
            sns.lineplot(x=coords[:, 0], y=coords[:, 1], ax=ax, linewidth=_lw(2.5))
            ax.set_title(f"L{idx}", fontsize=_fs(28))
            ax.set_xticks([])
            ax.set_yticks([])
            ax.set_aspect("equal", "box")
        fig3.suptitle(f"{act_name}", y=0.0, fontsize=_fs(28))
        fig3.tight_layout()
