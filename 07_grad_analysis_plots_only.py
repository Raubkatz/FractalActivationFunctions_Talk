...
New rebuild mode
----------------
- By default, this script rebuilds summaries and plots from previously saved per-run CSV files.
- Set REBUILD_FROM_SAVED = False to run the original expensive computation path again.
"""
...
# CONFIG (minimal)
# -----------------------------
#RESULTS_DIR = "results_grad_summary_regular"
#RESULTS_DIR = "results_grad_summary_extra"
#RESULTS_DIR = "results_grad_summary_weierstrass"
...
# Default: rebuild all plots/summaries from already saved CSV results.
# Set to False if you want to recompute gradients from scratch again.
REBUILD_FROM_SAVED = True

# If True, when rebuilding from saved CSVs, regenerate per-run plots/summaries too.
REBUILD_PER_RUN_ARTIFACTS = True


RESULTS_DIR_ = "results_grad_summary_others"
...
RESULTS_DIR_ = "results_grad_summary_regular"
...
# -----------------------------
def _write_run_summary(epoch_df: pd.DataFrame, out_dir: Path, dataset: str, act_name: str,
                       run_seed: int, neurons: int, batch_size: int, epochs: int):
    summary_lines = []
    ...
    if epoch_df.empty:
        ...
        if "grad_nonfinite_elems" in epoch_df.columns:
            nnf = pd.to_numeric(epoch_df["grad_nonfinite_elems"], errors="coerce").to_numpy(dtype=float)
            nnf_fin = nnf[np.isfinite(nnf)]
            if nnf_fin.size > 0:
                summary_lines.append(
                    f"grad_nonfinite_elems: min={nnf_fin.min():.6e}, max={nnf_fin.max():.6e}, mean={nnf_fin.mean():.6e}"
                )
            else:
        ...
        for col in ["grad_min", "grad_max", "grad_mean", "grad_std"]:
            if col not in epoch_df.columns:
                summary_lines.append(f"{col}: column missing.")
                continue
            v = pd.to_numeric(epoch_df[col], errors="coerce").to_numpy(dtype=float)
            vfin = v[np.isfinite(v)]
            if vfin.size > 0:
                summary_lines.append(
                    f"{col}: min={vfin.min():.6e}, max={vfin.max():.6e}, mean={vfin.mean():.6e}, std={vfin.std():.6e}"
                )
            else:
...
def _write_agg_summary(agg_df: pd.DataFrame, out_dir: Path, dataset: str, act_name: str,
                       neurons: int, batch_size: int, epochs: int):
    lines = []
    ...
    for base in ["grad_min", "grad_max", "grad_mean", "grad_std", "grad_nonfinite_elems"]:
        ...
        if col_mu in agg_df.columns:
            ...
            if mu_fin.size > 0:
                lines.append(
                    f"{base}_mean over epochs: min={mu_fin.min():.6e}, max={mu_fin.max():.6e}, mean={mu_fin.mean():.6e}"
                )
            else:
        ...
        if col_sd in agg_df.columns:
            ...
            if sd_fin.size > 0:
                lines.append(
                    f"{base}_std over epochs:  min={sd_fin.min():.6e}, max={sd_fin.max():.6e}, mean={sd_fin.mean():.6e}"
                )
            else:
...
def _extract_seed_from_run_csv_name(path: Path):
    m = re.match(r"run_(\d+)__epoch_grad_stats\.csv$", path.name)
    if m is None:
        return None
    return int(m.group(1))

def _load_saved_run_dfs_for_activation(out_dir: Path, dataset: str, act_name: str):
    """
    Load all previously saved per-run CSVs for one dataset/activation.
    Returns list[(run_seed, df)] sorted by run_seed.
    """
    if not out_dir.exists():
        return []

    rows = []
    for fp in sorted(out_dir.glob("run_*__epoch_grad_stats.csv")):
        run_seed = _extract_seed_from_run_csv_name(fp)
        if run_seed is None:
            continue
        ...
        if "activation" not in df.columns:
            insert_at = 1 if "dataset" in df.columns else 0
            df.insert(insert_at, "activation", str(act_name))
        if "run_seed" not in df.columns:
            insert_at = 2 if ("dataset" in df.columns and "activation" in df.columns) else 0
            df.insert(insert_at, "run_seed", int(run_seed))

        rows.append((int(run_seed), df))

    rows = sorted(rows, key=lambda t: t[0])
    return rows

def _rebuild_from_saved_for_activation(results_root: Path, dataset: str, neurons: int, batch_size: int,
                                       epochs: int, act_name: str):
    out_dir = results_root / "grad" / str(dataset) / str(act_name)
    _ensure_dir(out_dir)

    loaded = _load_saved_run_dfs_for_activation(out_dir, dataset, act_name)
    if len(loaded) == 0:
        print(f"[warn] No saved run CSVs found for dataset={dataset} act={act_name} in {out_dir}")
        return []

    run_dfs = []
    for run_seed, epoch_df in loaded:
        run_dfs.append(epoch_df)

        if REBUILD_PER_RUN_ARTIFACTS:
            _write_run_summary(
                epoch_df=epoch_df,
                out_dir=out_dir,
                dataset=str(dataset),
    ...
    if len(run_dfs) > 1:
        ...
        title = f"{dataset} | {act_name} | runs={len(run_dfs)} (probe batch gradients, mean across runs)"
...
def _main_recompute(results_root: Path):
    ...
    for (dataset, neurons, batch_size, epochs) in data_with_params:
        ...
        for (act_name, act_fn) in ACTIVATION_FUNCTIONS:
            ...
            for r in range(RUNS):
                seed = SEED_BASE + r
                print(f"[grad][compute] dataset={dataset} act={act_name} run={r+1}/{RUNS} seed={seed}")
                df_run = run_one(
...
def _main_rebuild_from_saved(results_root: Path):
    print("[grad][rebuild] Rebuilding plots/summaries from saved per-run CSV files.")
    print("[grad][rebuild] TensorFlow is imported for compatibility, but no model training is performed.")
    ...
    for (dataset, neurons, batch_size, epochs) in data_with_params:
        ...
        for (act_name, act_fn) in ACTIVATION_FUNCTIONS:
            print(f"[grad][rebuild] dataset={dataset} act={act_name}")
            run_dfs = _rebuild_from_saved_for_activation(
                results_root=results_root,
                dataset=str(dataset),
                neurons=int(neurons),
                batch_size=int(batch_size),
            ...
            # single-run representative table = first loaded run
            df0 = run_dfs[0].copy()
            if "activation" in df0.columns:
                df0 = df0.drop(columns=["activation"])
            df0.insert(0, "activation", str(act_name))
            ...
            # aggregate if multiple run CSVs exist
            if len(run_dfs) > 1:
                agg_df = _aggregate_runs(run_dfs, epochs=int(epochs))
...
def main():
    ...
    if REBUILD_FROM_SAVED:
        _main_rebuild_from_saved(results_root)
    else:
        _main_recompute(results_root)
