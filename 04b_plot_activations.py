...
def _plot(idx_list, suffix, out_dir):
    ...
    for k, idx in enumerate(idx_list):
        ...
        ax.plot(x, y, ls, color=color, lw=_lw(1.25), label=name)

    ax.set_xlabel(r"$x$", fontsize=_scaled_fontsize("axes.labelsize"))
    ax.set_ylabel(r"$\sigma(x)$", fontsize=_scaled_fontsize("axes.labelsize"))
    ax.tick_params(axis="both", which="both", labelsize=_scaled_fontsize("xtick.labelsize"), width=_lw(0.8))
